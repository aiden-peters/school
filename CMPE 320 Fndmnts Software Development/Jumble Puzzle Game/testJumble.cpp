...
void testJumble() {
	...
	//ADDED UNIT TESTS:
	//testing copy constructor attributes
	JumblePuzzle A("PLUM","easy");
	JumblePuzzle B(A);
	cout<<"\nCopy Constructor Attribute Testing: "<<endl;
	cout<<"Size Test ";
	bool sameLen = A.getSize() == B.getSize();
	if(sameLen){cout<<"Passed"<< endl;} else {cout<<"Failed"<<endl;}

	cout<<"Direction Test ";
	bool sameDir = A.getDirection() == B.getDirection();
	if(sameDir){cout<<"Passed"<< endl;} else {cout<<"Failed"<<endl;}

	cout<<"Row Test ";
	bool sameRow = A.getRowPos() == B.getRowPos();
	if(sameRow){cout<<"Passed"<< endl;} else {cout<<"Failed"<<endl;}

	cout<<"Column Test ";
	bool sameCol = A.getColPos() == B.getColPos();
	if(sameCol){cout<<"Passed"<< endl;} else {cout<<"Failed"<<endl;}

	//comparing using assignment operators
	cout << "\nTesting Assignment Copy Attributes: "<<endl;
	JumblePuzzle C = A;
	cout<<"Size Test ";
	sameLen = A.getSize() == C.getSize();
	if(sameLen){cout<<"Passed"<< endl;} else {cout<<"Failed"<<endl;}

	cout<<"Direction Test ";
	sameDir = A.getDirection() == C.getDirection();
	if(sameDir){cout<<"Passed"<< endl;} else {cout<<"Failed"<<endl;}

	cout<<"Row Test ";
	sameRow = A.getRowPos() == C.getRowPos();
	if(sameRow){cout<<"Passed"<< endl;} else {cout<<"Failed"<<endl;}

	cout<<"Column Test ";
	sameCol = A.getColPos() == C.getColPos();
	if(sameCol){cout<<"Passed"<< endl;} else {cout<<"Failed"<<endl;}
